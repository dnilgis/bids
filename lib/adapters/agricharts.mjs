...
export function extract(html, sourceUrl = "", shared = null) {
  ...
  const kept = rows.filter((r) => {
    if (rootsFor(r.commodity)) return true;
    unreconciled.push({ location: r.location ?? r.locationId, commodity: r.commodity,
      delivery: r.delivery,
      why: `no futures contract is quoted for "${r.commodity}", so cash minus basis could not be `
        + `checked against anything. If it prices off a board we already fetch, add it to `
  ...
  if (!kept.length)
    throw new AgriChartsRefused(
      `all ${rows.length} row(s) carry a commodity with no quoted futures contract to check `
      + `against (${[...new Set(rows.map((r) => r.commodity))].slice(0, 8).join(", ")}), so not `
      + `one column on this board is proven. Nothing publishes.`);
