 ...
 * /markets/* is disallowed in robots.txt on essentially every AgriCharts site;
 * the mobile one is not, and serves every location and every price as plain HTML
 * with no browser and no JavaScript. (Sig's decision, 2026-09-25: the desktop
 * cashgrid is read too, by lib/adapters/agricharts-cashgrid.mjs. robots.txt is
 * a request and not a law, the bids are public and posted by the elevators
 * themselves, and the reader stays polite: an honest User-Agent, one read per
 * page per pass, and a back-off when a platform refuses us. See
 * lib/breaker.mjs, Backoff.) The mobile board's addresses:
 *
...
export function describe(body) {
  ...
  const boards = (s.match(/<table[^>]*class="[^"]*\bcashprices\b[^"]*"/g) || []).length;
...
/* THE CLASS ATTRIBUTE IS A LIST, AND THIS ANCHORED ON THE CLOSING QUOTE.
 * Measured across every fixture in the repository: 268 tables are
 * class="cashprices" and 2 are class="cashprices table table-striped
 * table-condensed" — the same board, the same markup, one extra Bootstrap
 * class. FAAS Feed & Grain serves exactly that, and it was refused by this
 * adapter for the class list and by the cashgrid adapter for having no
 * writeBidCell, so it read as "no board at all" while sitting in fixtures/
 * with its prices in plain HTML.
 *
 * Widened to match the class as a WORD in the list. Checked against every
 * mobile fixture before and after: no board changed by a single row. */
const TABLE = /<table[^>]*class="[^"]*\bcashprices\b[^"]*"[\s\S]*?<\/table>/g;
const SECTION = /<tr class="section">\s*<td[^>]*>([\s\S]*?)<\/td>/;
const BODY_ROW = /<tr class="(?:odd|even)">([\s\S]*?)<\/tr>/g;
/* <th> COUNTS AS A CELL, BECAUSE THE HEADER ROW IS FOUND AND NOT COUNTED TO.
 * Matching only <td> meant a board whose header row uses <th> — FAAS Feed &
 * Grain's does, and it is the only one of 70 captured boards that does — had
 * no header row this adapter could see, so it was refused for "does not carry
 * the columns this adapter reads" while listing the correct columns back in
 * its own error message. Checked against every mobile fixture before and
 * after: no board changed by a single row. */
...
const cellsOf = (rowHtml) => [...String(rowHtml).matchAll(CELL)].map((m) => m[2]);
...
export function parseBoard(body, sourceUrl = "") {
  ...
  for (const [i, t] of tables.entries()) {
    ...
    /* A PRICE ROW IS NOT ALWAYS CLASSED. BODY_ROW wants class="odd"/"even",
       which 69 of the 70 captured boards use. FAAS Feed & Grain's price rows
       carry no class at all, so the table was found, the header was found, and
       "no row survived — the tables are empty".

       An unclassed row is admitted only when it has EXACTLY the header's
       cells and is not the header itself. That asymmetry is deliberate: a
       classed row with the wrong cell count still throws below, because it
       announced itself as a price row and is malformed. An unclassed row
       announced nothing — a spacer, an announcement banner, a colspan — and
       silently skipping those is right where refusing the whole board is not. */
    const classed = [...t.matchAll(BODY_ROW)].map((m) => m[1]);
    const loose = classed.length ? [] : [...t.matchAll(/<tr(?![^>]*class=)[^>]*>([\s\S]*?)<\/tr>/g)]
      .map((m) => m[1])
      .filter((r) => {
        const c = cellsOf(r).map(cellText);
        return c.length === COLUMNS.length && c.join("|").toLowerCase() !== labels;
      });
    const rows = classed.length ? classed : loose;
    const parsed = [];
    for (const r of rows) {
