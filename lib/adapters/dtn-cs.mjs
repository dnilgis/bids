...
export function extract(body, sourceUrl) {
  ...
  for (const b of list) {
    ...
    /* WHICH MONEY -- 2026-09-06. This field was in the payload from the first
       day and was read by nothing, and a board in Canadian dollars published
       for eight days as if it were US ones. `primaryPrice.currency` is the
       elevator's own unit; the top-level `currency` is what the widget renders
       after whatever the `units` parameter asked for. They have been identical
       on every record seen, and the guard above already refuses the row if the
       two prices diverge, so either is safe to take. The elevator's own is
       taken first because it is the one that is not a conversion. */
    const currency = String(b?.primaryPrice?.currency ?? b?.currency ?? "").trim() || null;
