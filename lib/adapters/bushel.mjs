...
function fromGetBidsList(data, body, sourceUrl) {
  ...
  for (const loc of locations) {
    ...
    for (const g of loc?.groups ?? []) {
      ...
      /* Stated per group, beside the unit, and read for the same reason. */
      const currency = String(g?.bidPriceCurrency ?? "").trim() || null;
  ...
  for (const loc of locations) {
    ...
    for (const c of loc?.crops ?? []) {
      ...
      for (const b of c?.bids ?? []) {
        ...
        /* THREE CURRENCY FIELDS, AND THEY HAVE TO AGREE -- 2026-09-06.
           This shape states the currency of the cash, of the basis and of the
           futures separately. A row where they differ is the same shape of
           problem as Riceland's per-CWT futures against a per-bushel cash: the
           subtraction is not between two numbers in the same thing. It is
           marked unreadable rather than guessed at, and `currency` is left null
           so resolveCurrency refuses the board rather than picking one. */
        const cashCur = String(b?.currency ?? "").trim().toUpperCase() || null;
        const basisCur = String(b?.basis_currency ?? "").trim().toUpperCase() || null;
        const futCur = String(b?.futures_currency ?? "").trim().toUpperCase() || null;
        const oneMoney = [basisCur, futCur].every((c) => c == null || c === cashCur);
        const currency = oneMoney ? cashCur : null;
