 ...
 * The first version matched the term shape with `\s*` sprinkled between every
 * token and required `\n\s*document.write` to close the body. It found 19
 * terms in the fixture and ZERO on the live page -- because the fixture was
 * rebuilt from a paste that had been through a chat window, and the live page
 * spaces and breaks those lines differently. A parser that agrees with a
 * hand-rebuilt copy of a page and not the page is not a parser.
 *
 * So: take the body by position, strip whitespace out of it entirely, and
 * match the resulting token stream. There is then no whitespace left to vary.
 * The `NoScrapeOffset` comment is an independent statement of the same number
 * and `offsetDisagreement()` checks the sum against it, so a looser match here
 * cannot quietly produce a wrong offset -- it produces a refusal. */
export function deriveOffset(html) {
  const head = html.match(/function\s+displayNumber\s*\(/);
  if (!head) return null;
  const start = head.index + head[0].length;
  const end = html.indexOf("document.write", start);
  if (end === -1) return null;

  const body = html.slice(start, end).replace(/\s+/g, "");
  const terms = [...body.matchAll(/x=\(-\(([+-])\(\(([\d.]+)\)\)\)\+x\)/g)];
...
/** The first stretch of displayNumber's body, whitespace removed. */
export function bodySample(html, n = 180) {
  const head = html.match(/function\s+displayNumber\s*\(/);
  if (!head) return "(no displayNumber)";
  const start = head.index + head[0].length;
  const end = html.indexOf("document.write", start);
  const body = html.slice(start, end === -1 ? start + 4000 : end).replace(/\s+/g, "");
  return JSON.stringify(body.slice(0, n)) + (body.length > n ? "…" : "") +
         (end === -1 ? " [NO document.write FOUND]" : "");
}
...
export function describe(html) {
  ...
  return [
    ...
    (calls > 1 && !d) ? `body starts: ${bodySample(html)}` : "",
