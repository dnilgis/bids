...
export function describe(html) {
  ...
  return [
    ...
    `DataGrid ${DATAGRID.test(html) ? "present" : "ABSENT"}`,
...
/* CLASS IS NOT ALWAYS THE FIRST ATTRIBUTE.
 *
 * This matched `<table class="DataGrid` — the class attribute in first
 * position — because that is how Flash Grain writes it:
 *
 *     <table class="DataGrid DataNormal" width="100%" >
 *
 * Run 91847384302 asked 38 AgHost sites and read NONE of them. Thirteen were
 * captured, and POET Grain and Glacial Lakes Energy put their board in:
 *
 *     <table name="cashbids-data-table" border="0" width="630"
 *            class="DataGrid DataGridPlus" align="center">
 *
 * — the same grid, the class fourth. describe() printed "DataGrid ABSENT"
 * about a page that plainly has one, which is the tell: an error message that
 * prints the right answer back at you is a selector bug, not a missing board.
 * That is the third time this repository has pinned an attribute position or a
 * whitespace shape and called the result a fact about somebody's website.
 *
 * Attribute ORDER is not part of the meaning of HTML. Match the class token
 * wherever it sits, and require the whole token: `DataGridPlus` on its own is
 * a different class and must not count as one.
 *
 * `[^>]{0,400}` and not `[^>]*` deliberately — a bounded scan cannot run away
 * across a malformed document looking for a `>` that never comes. */
export const DATAGRID =
  /<table\b[^>]{0,400}?\bclass=["'][^"']*\bDataGrid\b[^"']*["'][^>]{0,400}?>[\s\S]*?<\/table>/i;
 ...
 *
 * Flash Grain's grid puts the deliveries across the top (`<th scope="col">`) and
 * one row per measure (Cash Price / Basis / ...). POET, Glacial Lakes, Harmony,
 * E Energy Adams and Central Farm Service write the transpose: one row per
 * delivery, one COLUMN per measure, the measures named in the thead's own words
 * and NOT in the same order from site to site:
 *
 *   Harmony / Glacial / Bingham Lake  Delivery | Cash Price | Futures Month | Futures Price | Futures Change | Basis
 *   POET Ashton                       Delivery | Futures Month | Futures Price | Basis | Futures Change | Cash Price
 *   Central Farm Service              Del | Cash | Month | Basis | Chg | Cbot
 *   E Energy Adams                    (rowspan) | Basis | Cash Price | Futures Month | Futures Price | Futures Change | (blank)
 *
 * So the columns are read BY LABEL, never by position. Every label must be one
 * of the known words below; an unknown label, a repeated label, or a blank
 * column that carries a displayNumber() figure refuses the whole page. That is
 * what keeps Topflight's board (sixteen locations across, the cash column
 * unlabelled, `Basis` sixteen times) refused: reading it would mean guessing
 * which blank column is cash.
 *
 * WHAT THIS DOES NOT DO: it does not check cash - basis = futures. buildFile
 * does, on every row, from the source's own manifest; the adapter boundary is
 * parsing here and guarding there. A swapped label therefore surfaces as an
 * identity failure downstream, and test/aghost-delivery-rows.test.mjs proves it.
 *
 * NEVER INVENTED: the commodity is the page's own caption / commodity header or
 * the page is refused; the location is the header group's own text, or null
 * (and locationId null) when the page carries one unnamed location. */
const COLUMN_WORDS = new Map([
  ["delivery", "delivery"], ["del", "delivery"],
  ["cash price", "cash"], ["cash", "cash"],
  ["basis", "basis"],
  ["futures month", "month"], ["month", "month"],
  ["futures price", "price"], ["cbot", "price"],
  ["futures change", "change"], ["+/-", "change"], ["chg", "change"],
]);

const CELLS = /<t([dh])\b[^>]*>([\s\S]*?)<\/t\1>/gi;

/** Every DataGrid table on the page, whole. */
function allGrids(html) {
  const re = /<table\b[^>]{0,400}?\bclass=["'][^"']*\bDataGrid\b[^"']*["'][^>]{0,400}?>[\s\S]*?<\/table>/gi;
  return [...String(html).matchAll(re)].map((m) => ({ text: m[0], at: m.index }));
}

/** The commodity this grid is for, from the page's own words; null if none. */
function commodityOf(html, grid, prevEnd) {
  const cap = grid.text.match(/<caption[^>]*>([\s\S]*?)<\/caption>/i);
  if (cap && stripTags(cap[1])) return stripTags(cap[1]);
  const before = String(html).slice(prevEnd, grid.at);
  const ldp = [...before.matchAll(/<table\b[^>]*name=["']commodity-ldp["'][^>]*>([\s\S]*?)<\/table>/gi)].pop();
  if (!ldp) return null;
  const th = ldp[1].match(/<th\b[^>]*>([\s\S]*?)<\/th>/i);
  return th && stripTags(th[1]) ? stripTags(th[1]) : null;
}

/** The rows of every delivery-down-the-rows grid, or null when the page is not that layout. */
function extractDeliveryRows(html, off, sourceUrl) {
  const grids = allGrids(html);
  if (!grids.length) return null;
  const out = [];
  let seq = 0, prevEnd = 0;
  for (const grid of grids) {
    const t = grid.text;
    const thead = (t.match(/<thead[^>]*>([\s\S]*?)<\/thead>/i) || [, null])[1];
    if (!thead) return null;
    const hrows = [...thead.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)].map((m) => m[1]);
    if (!hrows.length) return null;
    const leaf = [...hrows[hrows.length - 1].matchAll(/<th\b[^>]*>([\s\S]*?)<\/th>/gi)].map((m) => stripTags(m[1]).toLowerCase());
    /* A rowspan'd th in an upper header row is the delivery column, absent from the leaf row. */
    const spans = hrows.length > 1 && /<th\b[^>]*rowspan=/i.test(hrows[0]);
    const cols = spans ? ["delivery", ...leaf.map((l) => (l === "" ? "" : COLUMN_WORDS.get(l) ?? "?"))]
                       : leaf.map((l) => (l === "" ? "" : COLUMN_WORDS.get(l) ?? "?"));
    if (cols.includes("?")) return null;                       // a label we do not know
    const named = cols.filter(Boolean);
    if (new Set(named).size !== named.length) return null;     // a repeated label (Topflight)
    for (const need of ["delivery", "cash", "basis", "month", "price"]) if (!named.includes(need)) return null;
    if (cols[0] !== "delivery") return null;                   // the delivery label leads the row

    const commodity = commodityOf(html, grid, prevEnd);
    prevEnd = grid.at + t.length;
    if (!commodity) throw new AghostRefused(`a delivery-rows DataGrid with no caption and no commodity header before it; the commodity is not invented. Got: ${describe(html)}`);
    /* The location is the header group's text, when there is one. */
    const grp = hrows.length > 1 ? hrows[0].match(/<th\b[^>]*colspan=["']?\d+[^>]*>([\s\S]*?)<\/th>/i) : null;
    const location = grp && stripTags(grp[1]) ? stripTags(grp[1]) : null;

    const body = t.slice(t.search(/<\/thead>/i));
    /* SPLIT ON THE OPENING <tr>, NOT ON A CLOSING ONE. E Energy Adams never
       writes </tr> between its rows (nineteen <tr>, no </tr> until the end), so
       a pair-matching regex read seventeen deliveries as one 119-cell row. */
    for (const tr of body.split(/<tr\b[^>]*>/i).slice(1).map((x) => x.split(/<\/tr>/i)[0])) {
      const cells = [...tr.matchAll(CELLS)].map((m) => m[2]);
      const figures = cells.filter((c) => /displayNumber\(/.test(c)).length;
      if (cells.length !== cols.length) {
        if (figures) throw new AghostRefused(`a delivery row has ${cells.length} cells under a ${cols.length}-column header and carries figures; which column is which is a guess. Got: ${describe(html)}`);
        continue;
      }
      const by = {};
      for (let i = 0; i < cols.length; i++) {
        if (!cols[i]) {
          if (/displayNumber\(/.test(cells[i])) throw new AghostRefused(`a figure sits in a column with no header. Got: ${describe(html)}`);
          continue;
        }
        by[cols[i]] = cells[i];
      }
      ...
      const cash = num(by.cash), basis = num(by.basis), price = parseEighths(by.price);
      const delivery = stripTags(by.delivery);
      if (!delivery || cash == null || basis == null || price == null) continue;  // blank row
      out.push({
        seq: seq++,
        location,
        locationId: location,
        ...
        futures: stripTags(by.month) || null,
        futuresPrice: price,
        futuresAt: null,
        futuresFlag: />s<\/span>/.test(by.price) ? "s" : null,
        source: sourceUrl,
        raw: `${location ?? ""} ${commodity} ${delivery}`.trim(),
      });
    }
  }
  return out;
...
export function extract(html, sourceUrl = "") {
  ...
  const grid = html.match(DATAGRID);
  ...
  if (!deliveries.length) {
    /* NOT THE FLASH GRAIN LAYOUT. Before refusing, ask whether this is the
       OTHER AgHost layout -- delivery down the rows -- which reads its columns
       from the header's own words. It returns null (and this refuses exactly as
       before) unless every column of every grid is a label it knows. */
    const rows = extractDeliveryRows(html, off, sourceUrl);
    if (rows) return rows;
    throw new AghostRefused(`no delivery columns in the DataGrid header. Got: ${describe(html)}`);
  }
