...
export function describe(html) {
  ...
  return [
    `${html.length} bytes`,
    `title "${title || "(none)"}"`,
    `displayNumber ${/function\s+displayNumber/.test(html) ? "present" : "ABSENT"}`,
    `DataGrid ${/<table class="DataGrid/.test(html) ? "present" : "ABSENT"}`,
    `aghost ${/aghost\.net/i.test(html) ? "present" : "ABSENT"}`,
    /cookieHelp|noCookiesWin/i.test(html) ? "COOKIE-HELP PAGE" : "",
    /loginForm|Forgot your password/i.test(html) ? "LOGIN FORM PRESENT" : "",
  ].filter(Boolean).join(" · ");
}
...
export function extract(html, sourceUrl = "") {
  ...
  if (!d) throw new AghostRefused(`no displayNumber() on the page, so nothing here is decodable. Got: ${describe(html)}`);
