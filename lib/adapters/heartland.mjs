...
export function extract(html, sourceUrl = "", shared = null) {
  ...
  /* A CELL POSTED AT EXACTLY ZERO BASIS SAYS NOTHING ABOUT THE COLUMN.
   *
   * This is NOT the "a zero is not a bid" rule discussed at the foot of this
   * function. That one is about zero CASH. This is zero BASIS against a real
   * cash price, and it is a different animal: cash minus a zero basis is the
   * cash price itself, so such a cell does not imply a futures price, it
   * restates its own. Feeding it to a test of "every cell here prices off ONE
   * contract" asks it a question it cannot answer, and it answers with itself.
   *
   * WHAT IT COST. From 2026-09-23 21:17 UTC -- minutes after the close --
   * every poll refused all 54 Heartland sources on one cell:
   *
   ...
   *
   * Seven cells agree at 1318c. The eighth is AGP St Joseph at 13.17 cash and
   * a basis printed as 0, so it implies 1317c -- its own cash, one cent from
   * where the column actually is. The board was refused for 21 hours and 54
   * Iowa locations went past the fourteen-hour staleness threshold and were
   * withdrawn from every consumer.
   *
   * THE SAME ACTION FITS BOTH READINGS OF THAT CELL, which is why this is safe
   * without knowing which is true. Either AGP is a processor delivery point
   * posted flat -- there is no basis because there is no basis -- or the board
   * prints 0 where it means "not stated". parseCell above requires two real
   * numbers and refuses the board otherwise, so this is a literal 0 on the
   * page either way. Flat or unstated, the cell carries no basis information
   * and must not be allowed to define the column.
   *
   * IT IS STILL PUBLISHED. Its cash price is a real posted price and goes out
   * with the rest. What it loses is a vote on what the column's futures
   * reference is.
   *
   * AND IT ONLY APPLIES WHILE SOMETHING IS LEFT TO CHECK AGAINST. If every
   * cell in a column is flat, there is no reference in it at all and the old
   * behaviour stands untouched -- they must still agree with each other, and a
   * column that disagrees with itself is still refused. This narrows the
   * check; it never switches it off. */
  const references = (group) => {
    const ref = group.filter((c) => c.basis !== 0);
    return ref.length ? ref : group;
  };
  ...
  for (const [, group] of byCol) {
    const voting = references(group);
    const counts = new Map();
    for (const c of voting) counts.set(c.impliedCents, (counts.get(c.impliedCents) ?? 0) + 1);
    if (counts.size === 1) continue;
    const [top] = [...counts.entries()].sort((a, b) => b[1] - a[1]);
    const odd = voting.filter((c) => c.impliedCents !== top[0]).slice(0, 4);
    throw new HeartlandRefused(
      `"${group[0].t.commodity}" ${group[0].col.delivery} (${group[0].col.code}): ` +
      `${voting.length} cell(s) imply ${counts.size} different futures prices -- ` +
  ...
  for (const [, group] of byCol) {
    const { col, t } = group[0];
    // The same exclusion, for the same reason: drift is measured against the
    // column's futures reference, and a flat-posted cell does not carry one.
    // Taking group[0] blindly would measure the drift of whichever cell the
    // page happened to print first.
    const anchor = references(group)[0];
    ...
    const drift = anchor.impliedCents - q.lastCents;
    if (Math.abs(drift) > MAX_DRIFT_CENTS)
      throw new HeartlandRefused(
        `"${t.commodity}" ${col.delivery} implies ${anchor.impliedCents}c from cash minus ` +
