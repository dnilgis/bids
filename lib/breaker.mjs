...
/* A rendered-widget read (lib/cdp.mjs captureRendered) that found no widget,
   could not load it, or watched it never finish drawing is the same signature:
   a page that gave us nothing to read. Those errors carry "[rendered]". */
export const loadedNothing = (msg) =>
  /did make 0 request\(s\)/.test(String(msg ?? "")) || /^\[rendered\]/.test(String(msg ?? ""));
...
/* BACK OFF WHEN A PLATFORM SAYS NO, INSTEAD OF ASKING TWO HUNDRED TIMES.
 *
 * The plain-HTTP reads (AgriCharts among them) had no reaction to being
 * refused. A 403 or 429 was one more line in that source's problems, and the
 * next source on the same platform was asked straight away. If a host starts
 * refusing us, the polite and the cheap response are the same: stop for the
 * rest of this pass. The next pass, ten minutes on, tries again.
 *
 * DISTINCT HOSTS, NOT REPEATS. One elevator's site answering 403 is that
 * elevator. Several different hosts on one platform in a row is the platform
 * or our address. So the strikes count different hosts, in a row, and any
 * good page resets them. Only 403 and 429 count; a 404 or a 500 is a site
 * problem and says nothing about whether we are welcome. */
export const isRefusalStatus = (s) => s === 403 || s === 429;

export class Backoff {
  constructor({ strikes = 4 } = {}) {
    this.limit = strikes;
    this.hosts = new Map();      // platform -> Set of hosts refusing in a row
    this.tripped = new Set();
  }
  /** A page came back with `status`. Returns true if this trips the platform. */
  note(platform, status, host) {
    if (!isRefusalStatus(status) || this.tripped.has(platform)) return false;
    const h = this.hosts.get(platform) ?? new Set();
    h.add(String(host ?? ""));
    this.hosts.set(platform, h);
    if (h.size >= this.limit) { this.tripped.add(platform); return true; }
    return false;
  }
  /** A page loaded. The run of refusals is broken. */
  ok(platform) { if (!this.tripped.has(platform)) this.hosts.set(platform, new Set()); }
  blocked(platform) { return this.tripped.has(platform); }
  culprits(platform) { return [...(this.hosts.get(platform) ?? [])]; }
}
