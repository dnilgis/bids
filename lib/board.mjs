  ...
  const file = {
    ...
    bids: corn
      ...
      .map((b, i) => ({
        ...
        /* A QUOTE WE COULD NOT VERIFY IS NOT PUBLISHED.
           No new field and no schema change: the one place this belongs is
           the quote itself, and it was already nullable. A row whose identity
           did not balance exactly goes out with no futures figure, so nothing
           downstream can print a number we could not check -- the Emmert
           pages already show a dash for a null. Their cash and their basis
           are untouched: those two agree with each other, and they are what
           a price is made of. */
        futuresPriceCents: offBy.has(b.delivery) ? null : (b.futuresPrice ?? null),
      })),
  };
