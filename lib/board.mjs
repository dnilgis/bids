...
export function validBand(range, where) {
  ...
  if (!(floor > 0))
    throw new Refused(`band for ${where} is [${floor}, ${ceiling}] -- the floor must be above ` +
                      `zero. A floor of 0 or below lets a zero or negative cash price pass the ` +
                      `level check as if it were a bid.`);
  if (!(floor < ceiling))
