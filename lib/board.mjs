...
export function bandFor(source, commodity) {
  ...
  if (source.bands) {
    for (const [name, range] of Object.entries(source.bands)) {
      if (key === name.toLowerCase() || key.includes(name.toLowerCase()))
        return { floor: range[0], ceiling: range[1], named: name };
    }
    return null;
  }
  if (typeof source.floor === "number" && typeof source.ceiling === "number")
    return { floor: source.floor, ceiling: source.ceiling, named: "default" };
  return null;
}
...
export function buildFile(html, { now, sourceUrl, source = CONFIG, extract = extractBids }) {
  ...
  const { kept, dropped, locations } = filterLocation(all, source.locationId);
  if (!kept.length)
  ...
  const corn = kept.filter((b) => source.expect.test(b.commodity || ""));
  if (!corn.length)
    throw new Refused(`no corn at ${source.location}. Page had: ` +
      `${[...new Set(kept.map((b) => b.commodity))].join(", ") || "nothing"}`);

  for (const b of corn) {
    if (b.cash == null) throw new Refused(`${b.delivery} has no cash bid`);
    const band = bandFor(source, b.commodity);
    if (!band)
      throw new Refused(`${b.commodity} has no configured band at ${source.location}. ` +
                        `Add one to the source before publishing it.`);
    if (b.cash < band.floor || b.cash > band.ceiling)
      throw new Refused(`${b.delivery} ${b.commodity} is ${b.cash}, outside ` +
                        `${band.floor} to ${band.ceiling} (${band.named} band)`);
  }
