...
export function delivery(raw, asOf) {
  ...
  /* TRUE WHEN THE MONTH THIS KEY NAMES HAS ALREADY GONE at the moment the
     board was read. Nothing about the key changes -- a season, a spot price, a
     month pair and an unreadable row are all `past: false`, because none of
     them names one month in the past. It exists so a consumer does not have to
     re-derive today from the key, which is how 37 rows reached three different
     AGSIST surfaces and were shown as prices a grower could take. */
  const out = (key, via, start = null, end = null) =>
    ({ key, start, end, via, past: isPastMonthKey(key, asOf), label });
  ...
  /* 2b. A CALENDAR DATE — "Sep 01, 2026", "October 15, 2026". Month, DAY, year,
        with the day sitting exactly where rule 5 expects the year.

        RULE 5 TAKES THE FIRST NUMBER AFTER THE MONTH NAME, so every one of these
        read as the year '01 — SEP 2001, NOV 2001, MAR 2001 — twenty-five years
        in the past. CDR Farms LLC of Bloomer, Wisconsin writes every row this
        way, and it did not refuse: `via` said "month-year" and the key looked
        like any other key, so nothing anywhere said a word. Found on 2026-09-05
        while working out why that elevator fell out of every delivery filter on
        agsist.com — sixteen miles from the reader who noticed.

        A FOUR-DIGIT YEAR IS REQUIRED and the day must be one or two digits, so
        this cannot swallow "Sept 26 Corn" (month and a two-digit year, which
        rule 5 reads correctly). The comma is optional because the shape is
        already unambiguous without it. The day is kept, as rule 2 keeps it. */
  const mdy = s.match(new RegExp(
    `\\b(${MONRX})(?![a-z])\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?,?\\s*(\\d{4})\\b`, "i"));
  if (mdy) {
    const m = MON[mdy[1].slice(0, 3)];
    const day = `${mdy[3]}-${pad(m)}-${pad(mdy[2])}`;
    return out(ym(mdy[3], m), "month-day-year", day, day);
  }

  /* 2c. A US NUMERIC DATE — "09/01/2026". Every AgriCharts mobile board writes
        its delivery this way, and it was reading as `unreadable`: 4,226 rows,
        MORE THAN THE 3,539 THE WHOLE FEED PUBLISHED, dropped at the merge guard
        for a shape nothing had been taught. Measured 2026-09-05 across every
        board file on disk.

        WHICH FIELD IS THE MONTH, PROVED RATHER THAN ASSUMED. Month/day order in
        a numeric date is a genuine ambiguity and this project does not guess at
        those, so it was counted:

            first field    01..12, all twelve present, NEVER above 12
            second field   01 on 4,090 of 4,226 rows, and also 03, 05, 07, 09,
                           15, 16 (124 rows) and 31 (3 rows)

        A field that reaches 31 is not a month and a field that never passes 12
        across four thousand rows in twelve distinct values is not a day. It is
        MM/DD/YYYY. A first field above 12 would mean a board writing the other
        order, which none does — so it is refused rather than swapped, because
        swapping on one row's evidence is the guess this rule exists to avoid.

        THE CONTRACT CODE IS NOT THE DELIVERY MONTH and is deliberately not
        consulted: "01/01/2027" arrives beside ZCH27, a January delivery priced
        off the March contract, which is ordinary. The date is the delivery. */
  const usDate = s.match(/\b(\d{1,2})\/(\d{1,2})\/(20\d{2})\b/);
  if (usDate) {
    const mm = +usDate[1], dd = +usDate[2], yy = usDate[3];
    if (mm >= 1 && mm <= 12 && dd >= 1 && dd <= 31) {
      const day = `${yy}-${pad(mm)}-${pad(dd)}`;
      return out(ym(yy, mm), "us-date", day, day);
    }
    return out(null, "unreadable-numeric-date-month-out-of-range");
  }
  ...
  /* A HYPHEN IS THE SAME PAIR AS A SLASH. "O-N 2026" and "O-N 2027" are on the
     boards beside "O/N 26"; two rows read as unreadable for the separator
     alone. Safe to widen because PAIR is a closed list of unambiguous initial
     pairs -- "J/J" is not in it and is still refused, because June/July and
     July/June are both readings and nothing on the line says which. */
  const pairLetters = s.match(/\b([a-z][\/-][a-z])\s*'?(\d{2,4})\b/);
  const pairKey = pairLetters ? pairLetters[1].replace("-", "/") : null;
  if (pairKey && PAIR[pairKey]) {
    const [a, b] = PAIR[pairKey];
  ...
  /* 4b. A DAY INSIDE A MONTH, WHICH IS NOT A YEAR. Rule 5 takes the first number
        after the month name and calls it the year, so nine boards had their
        delivery filed a decade or more in the past and nothing refused:
            ...
            by Sept 11  ->  2011-09      Inco Grain; Southwest Iowa Renewable
            by Sept 15th->  2015-09      Farmers Cooperative Society, Sioux Center
            By Nov 15th ->  2015-11      Ag Partners Cooperative, Red Wing

        These are half-month delivery windows and delivery deadlines. THE YEAR
        IS NOT ON THE ROW AT ALL, so it is inferred exactly as a bare month is
        — the next occurrence of that month, `via` saying it was inferred —
        rather than being taken from the day. Two shapes, both requiring
        something that a year cannot be:

          a RANGE   "11-20", "16-30": two day numbers, the second larger, both
                    1..31. "OCT-26" has one number and stays a month-year.
          a DEADLINE  the word "by" in front. "by Sept 11" is the eleventh of
                    September, and no board writes "by" in front of a year.

        Measured 2026-09-05 across 334 merged places: 10 carried a period
        outside 2026-2030, and every one of them is on this list. */
  /* THE ORDINAL SUFFIX IS PART OF THE SHAPE AND WAS NOT IN THE PATTERN.
     "Sept 21st-30th" is the same half-month window as "Sept 21-30", but the
     regex wanted a digit next to the dash and found "s", so the row fell
     through to rule 5 and had its DAY read as a year: 2021-09, on a board read
     in September 2026. ADM Columbus, Nebraska posts its corn this way.
     st/nd/rd/th are allowed after either number. They cannot appear on a year,
     so allowing them narrows nothing. */
  const dayRange = s.match(new RegExp(
    `\\b(${MONRX})(?![a-z])\\.?\\s*(\\d{1,2})(?:st|nd|rd|th)?\\s*[-–]\\s*(\\d{1,2})(?:st|nd|rd|th)?\\b(?!\\s*[-–]?\\s*\\d)`, "i"));
  if (dayRange) {
    const d1 = +dayRange[2], d2 = +dayRange[3];
    if (d1 >= 1 && d2 <= 31 && d2 > d1) {
      const m = MON[dayRange[1].slice(0, 3)];
      const yr = yearFor(m, asOf);
      if (!yr) return out(null, "unreadable-day-range-no-year");
      return out(ym(yr, m), "day-range-inferred-year",
                 `${yr}-${pad(m)}-${pad(d1)}`, `${yr}-${pad(m)}-${pad(d2)}`);
    }
  }
  const byDay = s.match(new RegExp(
    `\\bby\\s+(${MONRX})(?![a-z])\\.?\\s*(\\d{1,2})(?:st|nd|rd|th)?\\b(?!\\s*,?\\s*\\d)`, "i"));
  if (byDay) {
    const day = +byDay[2];
    if (day >= 1 && day <= 31) {
      const m = MON[byDay[1].slice(0, 3)];
      const yr = yearFor(m, asOf);
      if (!yr) return out(null, "unreadable-deadline-no-year");
      return out(ym(yr, m), "deadline-inferred-year", null, `${yr}-${pad(m)}-${pad(day)}`);
    }
  }
  ...
  if (mon) {
    ...
    if (mon[2]) {
      const y = year4(mon[2]);
      /* A TWO-DIGIT NUMBER THAT WOULD BE A PAST YEAR IS A DAY.
         "Sep 26" is September 2026. "Sept 25" is the twenty-fifth of
         September. Same shape, and this rule took the number as a year both
         times, so a board read in September 2026 published September 2025,
         2021 and 2016. Rules 4a and 4b take the two forms that carry proof
         they are days; the rest arrive here, where the only proof available is
         arithmetic: nobody quotes a cash bid for a month that has gone. Four
         digits are left alone -- a board that types "2026" in full is stale,
         not misread, and rewriting it would be guessing. */
      if (mon[2].length === 4 || !isPastYear(y, asOf)) return out(ym(y, m), "month-year");
    }
    const yr = yearFor(m, asOf);
...
/* The year a board was read in, or null when we were not told. */
function readYear(asOf) {
  if (!asOf) return null;
  const d = asOf instanceof Date ? asOf : new Date(asOf);
  return Number.isNaN(d.getTime()) ? null : d.getUTCFullYear();
}
/* Whether a candidate year is behind the year the board was read. Without an
   asOf nothing is behind anything, so this is false and the written year
   stands -- the same rule yearFor() follows when it refuses to invent one. */
function isPastYear(y, asOf) {
  const ry = readYear(asOf);
  return ry != null && y < ry;
}
/* Whether a YYYY-MM key names a month that has already gone. Only that shape:
   "spot", "newcrop-2026" and "2026-10/2026-11" are not one month and are never
   flagged. The current month counts as present, not past -- grain is still
   being delivered into it. */
function isPastMonthKey(key, asOf) {
  const m = /^(\d{4})-(\d{2})$/.exec(String(key || ""));
  if (!m || !asOf) return false;
  const d = asOf instanceof Date ? asOf : new Date(asOf);
  if (Number.isNaN(d.getTime())) return false;
  const now = d.getUTCFullYear() * 100 + (d.getUTCMonth() + 1);
  return (+m[1]) * 100 + (+m[2]) < now;
}
