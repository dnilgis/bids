...
/* WHICH MODES ARE STRICTLY NARROWER THAN WHICH — DERIVED, NOT TYPED.
 *
 ...
 * neither contains the other — that is a real ambiguity and gets no name.
 *
 * THIS TABLE USED TO BE TYPED OUT, AND THAT WAS A THIRD COPY OF A FACT
 * ALREADY IN THE CODE — 2026-09-07. `round-cent-both` was added to
 * CASH_ROUNDING and this table was not, so nothing knew it contained all four
 * of the others. Every board it explained came out AMBIGUOUS, five tests in
 * this file went red, and the mode was unusable — a fifth line here would have
 * been the fix and nothing would have stopped the sixth mode from repeating
 * it.
 *
 * So it is computed from the predicates themselves. That is exact rather than
 * approximate for a reason this file has already written down thirty lines
 * above: RESIDUALS LIVE ON AN EIGHTH-CENT GRID, because a futures quote is
 * quoted in eighths. Every mode is an interval on that grid, so walking the
 * grid IS the domain, not a sample of it. The walk runs to plus and minus two
 * cents, which is wider than the widest mode (round-cent-both, one cent each
 * way) and far past anything a display rounding can produce; a mode that
 * reached further would be a tolerance, not a rounding, and `containment`
 * asserts it does not.
 *
 * A contains B when every residual B explains is one A explains too, and A
 * explains at least one B does not. Equal modes are not containments in either
 * direction — two rules that accept exactly the same residuals are the same
 * rule under two names, and naming either would be arbitrary. */
export const RESIDUAL_GRID = (() => {
  const g = [];
  for (let e = -16; e <= 16; e++) g.push(e / 8);
  return g;
})();

export function containment(modes = CASH_ROUNDING, grid = RESIDUAL_GRID) {
  const accepts = {};
  for (const [name, rule] of Object.entries(modes)) {
    /* `exact` is declared null in CASH_ROUNDING — it means "use
       cashRoundingCents" and its own window is the float-noise budget. */
    const f = rule ?? ((s) => Math.abs(s) <= EXACT_TOL_CENTS);
    accepts[name] = new Set(grid.filter((s) => f(s)));
  }
  const out = {};
  for (const a of Object.keys(accepts)) {
    out[a] = Object.keys(accepts).filter((b) =>
      b !== a &&
      [...accepts[a]].every((s) => accepts[b].has(s)) &&
      accepts[b].size > accepts[a].size);
  }
  return out;
}

export const NARROWER_THAN = containment();
...
/* THE COMMITTED CAPTURE IS ALREADY IN CENTS — MEASURED 2026-09-08.
 *
 * `roundingEvidence` above wants the READER's row shape: cash and basis in
 * dollars, `futuresPrice` in cents. A committed `data/<id>.json` bid writes the
 * same three numbers under different names, and one of them is a trap.
 *
 * `futuresPriceCents` in a capture has ALREADY had the source's `futuresUnits`
 * scale applied — the file is the reader's OUTPUT, not the board's input.
 * Hillsdale's board prints its futures column as 5.385, its manifest declares
 * `futuresUnits: "dollars"`, and data/hillsdaleelevator-clinton.json says
 * 538.5. Applying the declared scale a second time on the way back in gives
 * residuals in the tens of thousands of cents; the first pass of this audit did
 * exactly that and reported the five sources that declare `dollars` as the
 * worst offenders in the repository. They were the only ones it had broken.
 *
 * So: no scale here, ever. If a future change makes captures store the board's
 * raw number instead, this function is the one place that has to know.
 *
 * A zero quote is dropped for the same reason buildFile drops it before
 * measuring units: a row with no futures number proves nothing about rounding,
 * and it would count as an `exact` hit against every mode. */
export function rowsFromCapture(capture) {
  return (capture?.bids ?? [])
    .filter((b) => b && b.cash != null && b.basisDollars != null
                && b.futuresPriceCents != null && b.futuresPriceCents !== 0)
    .map((b) => ({
      cash: b.cash,
      basis: b.basisDollars,
      futuresPrice: b.futuresPriceCents,
      commodity: b.commodity ?? null,
      delivery: b.delivery ?? null,
      identityCheckable: b.identityCheckable,
    }));
}
