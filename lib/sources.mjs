...
export const PLATFORMS = ["cashbidssingle", "aghost", "fragment", "graindesk", "dtn-cs", "bushel",
                          ...
                          /* Gradable — POET's 36 plants and ADM. */
                          "gradable",
                          /* Two elevators that publish their own bids.json under CC0.
                             See lib/adapters/emmert.mjs. */
                          "emmert-cash-bids",
                          /* Heartland Co-op's own closing board -- a matrix, one request for
                             54 Iowa locations. See lib/adapters/heartland.mjs. */
                          "heartland",
                          /* StoneHedge (StoneX) -- the cash-bid widget ten co-ops embed as an
                             iframe. Read from the widget's RENDERED document through a browser.
                             See lib/adapters/stonehedge.mjs. */
                          "stonehedge",
                          "first-party"];
...
export const PLATFORM_WIRE = {
  ...
  /* Gradable's board is JSON, and their bootstrap arrives with a `while(1);`
     hijacking prefix in front of it. The adapter strips it; the wire type is
     still json, because what comes back IS json and calling it html would put
     the wrong floor and the wrong sniffing on it. */
  gradable: "json",
  /* Heartland's board is a plain server-rendered .htm page -- no widget, no
     key, no JavaScript fetch. The capture is 50,890 bytes, so the 500-byte
     floor costs nothing and still catches a shell served in its place. */
  heartland: "html",
  /* Badger Grain and Midwest Commodity publish bids.json, and theirs is 1,830
     bytes with eleven rows in it -- comfortably over the 500-byte floor today,
     and exactly the shape that floor was written for: a one-location feed that
     gets smaller as deliveries roll off. json, because that is what comes back
     and because the adapter says something precise about a body that is not. */
  "emmert-cash-bids": "json",
  /* The rendered widget document, an HTML page of 35 to 240KB. */
  stonehedge: "html",
  "first-party": "json",
};
...
/* GRADABLE STAYS ON THE BROWSER, AND THE REASON CHANGED ON 2026-09-15.
 *
 * It is no longer "nobody has shown it need not be". Three runs showed it, on
 * BOTH partners, from a GitHub runner:
 *
 *     adm   16:56  probe 3/3, then 122 of 136 boards read in 62s
 *     adm   17:20  probe 3/3, then 122 of 136 boards read in 62s
 *     poet  18:00  probe 3/3, then  35 of  35 boards read in 12s
 *
 * "Read" is not "answered 200": scripts/gradable_boards.mjs runs every body
 * through this adapter's own extract() and counts rows, so a shell page or a
 * login wall fails there. The fourteen ADM boards that did not read failed on
 * their own contents, and not one failed to arrive.
 *
 * THE LINE BELOW THIS COMMENT USED TO PROMISE "moving it here is one line and
 * saves the load". THAT PROMISE WAS WRONG, and changing the line is how it was
 * found: ten tests went red. A `fetch` source MAY NOT CARRY browserPage —
 * validateSource refuses it a hundred lines down, "has browserPage but platform
 * is not read through a browser, so nothing would use it" — and all
 * THIRTY-FIVE committed POET manifests carry one. The move is one line plus
 * thirty-five live files, and relaxing that guard to avoid them would be
 * softening a check that is doing its job.
 *
 * SO IT IS STILL BROWSER, AND THAT NOW HAS A COST WORTH NAMING: one page load
 * per source at about 45 seconds. ADM's 122 markets is ninety minutes against a
 * ten-minute cadence, so on this transport those manifests can never be polled
 * at all. The transport move and the thirty-five manifests are one change and
 * belong in one kit, with the POET files rewritten and re-read. */
/* THE EVIDENCE, KEPT WHERE THE DECISION IS.
 *
 * What stood here said: "Its board was read in run 92448519432 the way the
 * probe reads everything ... The instruments URL carries no key and says
 * `offer_type=public`, so a plain GET might well work — but their bootstrap
 * body opens `while(1);`, their page also calls /api/effu/user, and this
 * sandbox cannot reach gradable.com to find out. Declaring it `fetch` on that
 * reasoning would be a guess that fails as 0 bids at six in the morning." And
 * then: "If a run ever proves a plain GET returns the same JSON, moving it here
 * is one line and saves the load."
 *
 * Three runs proved it, on BOTH partners, from a GitHub runner:
 *
 *     adm   17:20  probe 3/3, then 122 of 136 boards read in 62s
 *     adm   16:56  probe 3/3, then 122 of 136 boards read in 62s
 *     poet  18:00  probe 3/3, then  35 of  35 boards read in 12s
 *
 * "Read" is not "answered 200": scripts/gradable_boards.mjs runs every body
 * through this adapter's own extract() and counts the rows, so a shell page or
 * a login wall is a failure there, not a pass. The fourteen ADM boards that did
 * not read failed on their own contents — nine returned an empty instruments
 * array and five carry rows with no futures price — and not one failed to
 * arrive.
 *
 * WHY THIS IS NOT A LUXURY. Browser costs a page load per source, about 45
 * seconds. POET's 35 sources are survivable that way. ADM's are 122, which is
 * ninety minutes against a ten-minute cadence: on the browser those manifests
 * could never be polled at all, whatever else was true about them.
 *
 * WHAT IT COSTS IF THIS IS WRONG. A source that cannot be read keeps its last
 * good file and stays published inside the withdrawal window, so a transport
 * that stopped working is a stale price and a loud failure, never a wrong one.
 * And it is one line back. */
export const PLATFORM_TRANSPORT = { "dtn-cs": "browser", bushel: "browser", gradable: "browser",
                                    /* StoneHedge draws its prices with React from a socket nothing
                                       recorded; the widget's own document is an empty shell (5,858
                                       bytes, the same for every customer). Only the rendered page has
                                       them. */
                                    stonehedge: "browser" };
export const transportOf = (platform) => PLATFORM_TRANSPORT[platform] ?? "fetch";

/* WHAT A BROWSER SOURCE HANDS BACK. Every browser platform above returns the
   BODY of one response the page asked for (lib/cdp.mjs capture). StoneHedge has
   no such response: the board is the DOM after the widget drew it, so it is read
   by captureRendered, which loads the customer's page, finds the widget address
   that page embeds, and returns the rendered document. `url` is then the widget's
   address WITHOUT the key (the iframe to look for), and `browserPage` the
   customer's page that embeds it. */
export const PLATFORM_CAPTURE = { stonehedge: "rendered" };
export const captureOf = (platform) => PLATFORM_CAPTURE[platform] ?? "response";
...
/* Which money a board may be in, and what places a source in a country. The
   list lives in lib/currency.mjs so the validator and the reader cannot come
   to different answers about it. */
import { CURRENCIES, COUNTRIES, CURRENCY_OF_COUNTRY, countryOfState } from "./currency.mjs";

/* ONE LIST, NOT TWO -- 2026-09-07.
 *
 * These were typed out here as literals while lib/board.mjs held the rules
 * themselves, so the validator and the reader each had their own idea of what
 * a legal value was. Adding `round-cent-both` to board.mjs proved it: the mode
 * worked perfectly in checkIdentity and every manifest that declared it was
 * rejected on load by this file, with a message listing four modes that no
 * longer described the code.
 *
 * The comment two lines above already states the rule for CURRENCIES -- "the
 * list lives in lib/currency.mjs so the validator and the reader cannot come
 * to different answers about it" -- and these two were the exception to it.
 * Derived now, so a mode cannot exist in one file and not the other. */
import { CASH_ROUNDING, FUTURES_UNITS as FUTURES_UNIT_SCALES } from "./board.mjs";

export const CASH_ROUNDING_MODES = Object.keys(CASH_ROUNDING);
export const FUTURES_UNITS = Object.keys(FUTURES_UNIT_SCALES);
...
/* Where a source's coordinate may be, by country. Wide enough for the whole
   growing area and narrow enough that a swapped pair falls out. */
export const COORD_BOX = {
  US: { lat: [24, 50], lon: [-125, -66], what: "the continental US" },
  /* Middle Island ON is 41.7N and the Yukon reaches -141; Newfoundland reaches
     -52. Farmland stops well short of 70N, which is the ceiling here. */
  CA: { lat: [41, 70], lon: [-141, -52], what: "Canada" },
};
...
export function validateSource(s, seenIds = new Set()) {
  ...
  /* THE BOX IS PER COUNTRY, AND IT USED TO BE THE UNITED STATES ONLY — 2026-09-15.
   *
   * The check below exists to catch a transposed pair: a latitude and longitude
   * swapped read as plausible numbers and land in the wrong hemisphere. It did
   * that by asserting the continental US, on a feed that already publishes
   * Canadian boards. Two Ontario sources pass it TODAY only by accident —
   * addisgrain-oromedonte at 44.45 and sharedonfarms-owensound at 44.52 sit
   * inside the US latitude band because southern Ontario is south of Minnesota.
   *
   * ADM's bootstrap is where that ran out. Five of their markets — Calgary and
   * Lloydminster AB, North Battleford, Vanscoy and Watson SK — carry ADM's own
   * geocoded coordinate at latitudes 51 to 58, and this line refused every one.
   * Not the row: the MANIFEST, and loadSources validates before it skips a
   * disabled source, so five held-disabled files would have put five errors
   * into every pass for the whole feed.
   *
   * So the box is chosen by the source's country — its own if it declares one,
   * otherwise the one its state code implies. NOTHING IS LOOSENED FOR A US
   * SOURCE: the US box is the same four numbers it always was. The Canadian box
   * is Canada, and the transposition it was written to catch is still caught —
   * swapping Calgary's pair gives a latitude of -114, which is not a latitude
   * at all and is outside every box here. */
  if (typeof s.lat === "number" && typeof s.lon === "number") {
    const country = s.country ?? countryOfState(s.state) ?? "US";
    const box = COORD_BOX[country] ?? COORD_BOX.US;
    if (!(s.lat > box.lat[0] && s.lat < box.lat[1] && s.lon > box.lon[0] && s.lon < box.lon[1]))
      bad.push(`${id}: ${s.lat},${s.lon} is not in ${box.what}. A transposed pair reads as a plausible number and lands in the wrong hemisphere.`);
  }
  ...
  /* A CURRENCY THAT IS NOT ONE OF OURS IS A TYPO OR A COUNTRY WE HAVE NOT DONE
     THE WORK FOR, AND EITHER WAY IT MUST NOT PUBLISH. Adding a third currency
     is not one line here: it needs a band set in that currency, a consumer that
     renders it, and somebody who has looked at a board in it. */
  if (s.currency !== undefined && s.currency !== null && !CURRENCIES.includes(s.currency))
    bad.push(`${id}: currency "${s.currency}" is not one of ${CURRENCIES.join(", ")}. ` +
      `Adding one is a piece of work, not a string.`);
  if (s.country !== undefined && s.country !== null && !COUNTRIES.includes(s.country))
    bad.push(`${id}: country "${s.country}" is not one of ${COUNTRIES.join(", ")}`);
  /* THE TWO MUST AGREE WITH EACH OTHER AND WITH THE STATE. A source stamped
     country US and currency CAD is a stamping bug, and it would publish. */
  if (s.country && s.currency && CURRENCY_OF_COUNTRY[s.country] !== s.currency)
    bad.push(`${id}: country ${s.country} and currency ${s.currency} disagree`);
  if (s.state && s.country && countryOfState(s.state) &&
      countryOfState(s.state) !== s.country)
    bad.push(`${id}: state ${s.state} is in ${countryOfState(s.state)}, not ${s.country}`);
  return bad;
}
...
export function toConfig(s) {
  return {
    ...
    /* WITHOUT THESE TWO, buildFile CANNOT ANSWER "which money". Same failure
       mode as cashRounding and futuresUnits before 2026-08-20: the field was
       in the manifest, board.mjs read it, and nothing carried it across, so
       the declaration silently did nothing. */
    state: s.state ?? null,
