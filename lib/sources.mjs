...
/* GRADABLE IS ON THE BROWSER BECAUSE NOBODY HAS SHOWN IT NEED NOT BE.
 *
 * Its board was read in run 92448519432 the way the probe reads everything: a
 * real Chromium on poet.gradable.com/market/Big-Stone-City--SD, watching what
 * their own page asked for. The instruments URL carries no key and says
 ...
 * reasoning would be a guess that fails as "0 bids" at six in the morning.
 *
 * Browser costs a page load and works either way. If a run ever proves a plain
 * GET returns the same JSON, moving it here is one line and saves the load. */
