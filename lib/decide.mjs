...
/* CHANGED 2026-08-27, 6 -> 0.5. Sig: "i want the prices to be updated with a
   timestamp even if there was no change to the price, so then i dont wonder if
   it is broken or not, because the timestamp will be more frequent."

   He is describing the exact hole this constant opened. On 2026-08-26 the
   reader ran a pass every eleven minutes from 22:10 to 00:13 and every one of
   them was healthy -- and data/boyceville.json sat at checkedAt 18:36:19Z the
   whole time, because the price had not moved and six hours had not passed.
   Anyone reading the per-source file, which is what the two Emmert sites read,
   saw a five-hour-old stamp on a feed that was working perfectly. A freshness
   signal that stops moving while the system is healthy is not a freshness
   signal; it is a source of false alarm, and worse, it cannot be told apart
   from the real thing.

   WHY 0.5 AND NOT 0. Every pass already produces a commit -- index.json
   carries `generated` and that changes on every run -- so writing the source
   files costs no extra commits, only a bigger diff. Measured, by building a
   255-file repo of the real shape and committing twenty timestamp-only
   changes: about 20 KiB of pack per commit, or roughly 470 MiB a year at the
   current cadence. That matters here and only here: one-pass.sh does
   `git fetch --deepen=1000` because the dashboard's basis chart is drawn from
   this repo's history, so the history can never be pruned and every poll pays
   for its size. Half an hour caps visible staleness at about 30 minutes -- if
   a stamp is older than that during the day, something IS wrong, which is the
   whole point -- at roughly 40% of the every-pass cost.

   To make it every pass, set this to 0. Nothing else needs to change. */
export const HEARTBEAT_H = 0.5;
